...
def get_turns(start, end):
    ...
    right_clockwise = cycle([1,2,3,4,5,6,7,8])
    left_clockwise = cycle([8,7,6,5,4,3,2,1])
    right = turn_count(right_clockwise, start, end)
    left = turn_count(left_clockwise, start, end)
    if left < right:
